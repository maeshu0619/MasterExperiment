...
class AddModule(nn.Module):
    ...
    def forward(self, pts, feats, density_score, structure_score):
        ...
        for b in range(B):
            ...
            """
            # 上限（max_add_ratio）と下限（target_add_ratio）の整合を取る
            max_add_points = max(1, int(self.max_ratio * N))
            # 全ゼロ崩壊対策の下限。ただし上限は超えない
            min_add = max(1, int(self.target_add_ratio * N))
            min_add = min(min_add, max_add_points)  # ← これが重要（0.05を絶対に破らない）

            # いったんmask由来のidxを作った後、必ず [min_add, max_add_points] に収める
            k = int(idx.numel())

            if k < min_add:
                idx = torch.topk(add_prob[b], k=min_add, largest=True).indices
            elif k > max_add_points:
                idx = torch.topk(add_prob[b], k=max_add_points, largest=True).indices
            """
        ...
        add_prob_sel = torch.gather(add_prob, dim=1, index=add_idx)  # (B,Kmin)
        L_fit = self._compute_L_fit(pts, new_pts, add_prob=add_prob_sel)
        ...
        if self.cfgs.trainORtest == "train":
            ...
            add_ratio_real = float(new_pts.shape[2]) / float(pts.shape[2])
            ...
            if self.writer is not None and hasattr(self.writer, "write"):
                self.writer.write(
                    f"L_add   :{L_add:.4f}->L_cnt:{L_cnt:.4f}, L_fit:{L_fit:.4f}, L_rep:{L_rep:.4f}, "
                    f"AddRatio(exp):{mean_add_ratio.mean().item():.6f}, "
                    f"AddRatio(real):{add_ratio_real:.6f}, "
                    # f"min_add:{min_add}, max_add:{max_add_points}, Kmin:{new_pts.shape[2]}"
                )
