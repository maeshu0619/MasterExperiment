...
def compress(args, oct_data_seq, model, writer):
    model.eval()
 ...
 # %%
def oa_main(args, pts, model, qs, writer, file_date):
    pts_np = (
       pts
       .detach()
       .squeeze(0)      # (3, N)
    ...
    saveMatDir=f'compress/OctAttention/encoded'
    if not os.path.exists(saveMatDir):
        os.makedirs(saveMatDir)
    matFile,DQpt,refPt = dataPrepare(args, pts_np, saveMatDir,qs=qs,ptNamePrefix='',rotation=False)
    matDataPath = matFile
    ...
    _oct_seq_stats(oct_data_seq, writer=None)
    single_child_stats_from_oct_seq(oct_data_seq, writer=None)
    single_ratio = single_child_by_level(oct_data_seq, writer=None)
    popcount_histogram(oct_data_seq, writer=None)


    ptNum = pts.shape[2]
    # writer.write(f"[OA] Number of points: {ptNum}\n")
    
    binsz, oct_len, elapsed, binszList, octNumList = compress(args, oct_data_seq, model, writer)
