...
def extract_knn_patch(k, pts, center_pts, return_idx=False):
    ...
    Args:
        k           : パッチ内点数
        pts         : (1, 3, N) 入力点群
        center_pts  : (1, 3, M) パッチ中心点
        return_idx  : True の場合、KNNインデックスを返す

    Returns:
        patches     : (M, 3, k)
        knn_idx     : (M, k)  ※ return_idx=True の場合のみ
    """
    ...
    # パッチ点群取得
    # ----------------------------
    patches = np.take(pts_np, knn_idx, axis=0)   # (M, k, 3)
    patches = torch.from_numpy(patches).float().cuda()
    patches = rearrange(patches, 'm k c -> m c k').contiguous()

    if return_idx:
        # torch Tensor に変換して GPU に載せる
        knn_idx = torch.from_numpy(knn_idx).long().cuda()
        return patches, knn_idx
